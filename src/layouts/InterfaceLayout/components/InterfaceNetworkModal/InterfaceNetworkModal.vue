...
<div class="modal-container">
  <b-modal ref="network" :title="$t('interface.network')" hide-footer centered class="bootstrap-modal network nopadding max-height-1" static lazy>
      ...
      <div class="flex-container">
        ...
        <div class="margin-left-auto add-custom-network">
          ...
          <div class="sliding-switch-white">
            <label class="switch">
              <input ref="addCustomToggle" type="checkbox" @click="addCustomNetworkToggle" />
    ...
    <div ref="networkList" class="network-list">
      <div v-for="(key, index) in Object.keys(reorderedNetworks)" :key="key + index" class="content-block">
        ...
        <div class="grid-3">
          <p v-for="net in Networks[key]" :key="net.service" :class="
                net.service === network.service &&
                net.type &&
                net.type.name === network.type.name
                  ? 'current-network'
                  : ''
              " class="switch-network" @click="switchNetwork(net)">
            {{ net.service }}
          </p>
        </div>
      </div>
      <div v-if="customNetworks.length > 0" class="content-block">
        ...
        <div v-for="(net, idx) in customNetworks" :key="net.service + '(' + net.type.name + ')' + idx" class="grid-3">
          <div :class="
                net.service === network.service &&
                net.type.name === network.type.name
                  ? 'current-network'
            ...
            <i class="fa fa-times-circle" @click.prevent="removeNetwork(net, idx)" />
          </div>
        </div>
      </div>
    </div>
    <form ref="networkAdd" class="network-add hidden">
      <div class="content-block">
        <div class="input-block-container">
          <input v-model="name" v-validate="'required'" class="custom-input-text-1" type="text" name="nodeName" placeholder="ETH Node Name" autocomplete="off" />
          <select v-model="selectedNetworkName" class="custom-select-1">
            <option v-for="type in types" :key="type.name + type.name_long" :value="type.name" :selected="selectedNetworkName === type.name">
              {{ type.name | capitalize }} -
              {{ type.name_long | capitalize }}
            </option>
          </select>
          <input v-model="url" v-validate="{
                required: true,
                url: {
              ...
              }" class="custom-input-text-1" type="text" name="nodeUrl" placeholder="URL" autocomplete="off" />
          <input v-model="port" class="custom-input-text-1" type="number" name="nodePort" placeholder="Port" autocomplete="off" />
          <input v-show="selectedNetworkName === 'CUS'" v-model="blockExplorerTX" v-validate="'required|url:require_protocol'" class="custom-input-text-1" type="text" name="customExplorerTx" placeholder="https://etherscan.io/tx/[[txHash]]"
            autocomplete="off" />
          <input v-validate="'required|numeric'" v-show="selectedNetworkName === 'CUS'" v-model="chainID" class="custom-input-text-1" type="number" name="customChain" placeholder="Chain ID" autocomplete="off" />
          <input v-validate="'required|url:require_protocol'" v-show="selectedNetworkName === 'CUS'" v-model="blockExplorerAddr" class="custom-input-text-1" type="text" name="customExplorerAddr" placeholder="https://etherscan.io/address/[[address]]"
            autocomplete="off" />
        </div>
        <div>
      ...
      <div class="content-block">
        ...
        <div ref="authForm" class="auth-form-container hidden">
          <input v-model="username" class="custom-input-text-1" type="text" name placeholder="User Name" autocomplete="off" />
          <input v-model="password" class="custom-input-text-1" type="password" name placeholder="Password" autocomplete="off" />
        </div>
      </div>

      <div class="content-block">
        <div class="save-button-container">
          <button v-show="selectedNetworkName !== 'CUS'" :class="[
                errors.has('nodeName') ||
                errors.has('nodeUrl') ||
                url === '' ||
                name === ''
          ...
          <interface-bottom-text :link-text="$t('interface.helpCenter')" :question="$t('interface.dontKnow')" link="https://kb.myetherwallet.com" />
        </div>
      </div>
    </form>
  </b-modal>
...
export default {
  ...
  data() {
    return {
      types: networkTypes,
      selectedNetworkName: 'ETHO',
      chainID: networkTypes['ETHO'].chainID,
      port: 443,
      name: '',
      url: '',
      username: '',
    ...
    removeNetwork(net, idx) {
      ...
      if (net.service === this.network.service) {
        if (this.customNetworks.length > 0) {
          ...
          this.switchNetwork(this.Networks.ETHO[0]);
        }
      }
